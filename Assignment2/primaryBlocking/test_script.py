...
read_response1 = client1.read(file1_uuid,REPLICA_ADDRESSES[1])
assert read_response1.status.type == Status.STATUS_TYPE.SUCCESS, "read status should be SUCCESS"
assert read_response1.file_name == FILE1_NAME, "read file name not correct"
assert read_response1.content == FILE1_CONTENT, "read file content not correct"
...
assert read_response2.status.type == Status.STATUS_TYPE.FAILURE, "read status should be FAILURE"
