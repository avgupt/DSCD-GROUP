    ...
    def writeFromClient(self, request, context):
        ...
        if self.is_primary_replica:
            ...
            if (request.uuid not in self.key_value_pairs.keys() and not self.filenameInPairs(request.file_name)) or (request.uuid in self.key_value_pairs.keys() and self.filenameInPairs(request.file_name) and self.key_value_pairs[request.uuid] == request.file_name):
                ...
                file_version = self.writeInFile(request.file_name, request.file_content)
                self.key_value_pairs[request.uuid] = (request.file_name, file_version)
                response = server_pb2.WriteResponse(uuid=request.uuid, version=file_version, status=Status(type=Status.STATUS_TYPE.SUCCESS))
                yield response
                ...
                # print("Cse 2")
                yield server_pb2.WriteResponse(uuid=request.uuid, status=Status(type=Status.STATUS_TYPE.FAILURE, message="FILE WITH THE SAME NAME ALREADY EXISTS"))
                ...
                # print("case 4")
                yield server_pb2.WriteResponse(uuid=request.uuid, status=Status(type=Status.STATUS_TYPE.FAILURE, message="DELETED FILE CANNOT BE UPDATED"))
                ...
                yield server_pb2.WriteResponse(uuid=request.uuid, status=Status(type=Status.STATUS_TYPE.FAILURE, message="FILE UUID AND FILE NAME DO NOT MATCH"))

            else:
                # Case else
                yield server_pb2.WriteResponse(uuid=request.uuid, status=Status(type=Status.STATUS_TYPE.FAILURE, message="INVALID CASE"))
            
        else:
            with grpc.insecure_channel(str(self.primary_replica_ip)+ ":" +str(self.primary_replica_port), options=(('grpc.enable_http_proxy', 0),)) as channel:
                stub = server_pb2_grpc.ServerServiceStub(channel)       
                response_stream = stub.writeFromClient(server_pb2.WriteRequest(file_name=request.file_name, file_content=request.file_content, uuid=request.uuid))
                response = next(response_stream)
                response_stream.cancel()
                if response.status.type != Status.STATUS_TYPE.SUCCESS:
                    yield server_pb2.WriteResponse(uuid=request.uuid, status=response.status)
                else:
                    yield server_pb2.WriteResponse(uuid=request.uuid, version=response.version, status=response.status)
